...
// This code generates scattering intensity patterns and orientation tensors uu,uuuu for the FFoRM streamlines including upstream flow
int main(int argc,const char *argv[]){
	sscanf(argv[1],"%lf",&FRR);
	sscanf(argv[2],"%lf",&Q2);
	sscanf(argv[3],"%d",&strm);
	// sscanf(argv[4],"%d",&nqi);
	// sscanf(argv[5],"%lf",&qlim);
	sscanf(argv[4],"%d",&nang);
	sscanf(argv[5],"%d",&ntraj);
	// sscanf(argv[6],"%d",&flow_steps);
	sprintf(flow_input,"../Gamma_input/FRR_%.2f/Q2_%.2f/strm_%d.txt",FRR,Q2,strm);
	printf("input file %s\n",flow_input);
	...
	sprintf(output,"/output/FRR_%.2f",FRR);
	strcat(outdir,output);
	result = mkdir(outdir, 0777);
	sprintf(output,"/Q2_%.2f",Q2);
	strcat(outdir,output);
	result = mkdir(outdir, 0777);
	sprintf(output,"/strm_%d",strm);
	strcat(outdir,output);
	result = mkdir(outdir, 0777);
	...
	sprintf(output,"/h_na_%d.txt",nang);
	strcat(outdir,output);
	Dr = 2.0;
	for(i=0;i<flow_steps;++i){
		tf[i] *= Dr;
	...
	tmax = (int)(tend/dt);
	// Run for 5 rot diff times with initial velocity gradient tensor before evolution along streamline
	tinit = 5.0/Dr;
	tsini = (int)(tinit/dt);
	printf("Init time steps %d No. time steps %d\n",tsini,tmax);
	traj_time_print = 10000;
	...
	*idum = initRan();
	...
	for(nt=0;nt<ntraj;++nt){
		...
		un[0] = sin(theta0)*cos(phi0);
		un[1] = sin(theta0)*sin(phi0);
		un[2] = cos(theta0);
		// t_elap = 0.0;
		f_step = 0;
		for(t=1;t<(tmax + tsini);++t){
			// outfile = fopen(outdir,"a");
	...
	getcwd(outdir,4096);
	printf("output dir %s\n",outdir);
	sprintf(output,"/output/FRR_%.2f/Q2_%.2f/strm_%d/uu_u4/",FRR,Q2,strm);
	strcat(outdir,output);
	// strcat(outdir,output);
	result = mkdir(outdir, 0777);
	// sprintf(output,"/h_na_%d_nq_%d_qlim_%.2f.txt",nang,nqi,qlim);
	...
	getcwd(outdir,4096);
	// printf("output dir %s\n",outdir);
	sprintf(output,"/output/FRR_%.2f/Q2_%.2f/strm_%d/uu_u4/u4_%d.txt",FRR,Q2,strm,ntraj);
	strcat(outdir,output);
	outfile = fopen(outdir,"w");
	fprintf(outfile,"t");
	for(i=0;i<3;++i){
	...
	// The calculation of the scattering intensity from the OPDF is performed in Python for computational efficiency due to the ordering of the loops and matrix math
	// However, the calculation in C is included below if preferred
	// The output file probably needs to be corrected for lines 405-412 to output Pq
	// outfile = fopen(outdir,"a");
