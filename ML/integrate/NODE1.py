...
parser.add_argument('--dPCA', type=int, default=30) # Number of PCA modes retained
...
if __name__ == '__main__':
    ...
    print('mean, std')
    print(a_mean,a_std)
    pickle.dump([a_mean,a_std],open('a_stats_v2_%d.p'%dPCA,'wb'))
    ...
    # print('mean, std')
    # print(a_mean,a_std)
    # pickle.dump([a_mean,a_std],open('a_stats_%d.p'%dPCA,'wb'))
    ...
    # # print('mean, std')
    # # print(u_mean,u_std)
    # u_mean,u_std = pickle.load(open('stats.p','rb'))

    # Convert to torch tensor
    a = torch.tensor(anp[:data_size,np.newaxis,:])
    ...
    if args.load==0 or args.load==2:
        ...
        for itr in range(itr_start, args.niters + 1):
            ...
            optimizer.zero_grad()
            batch_y0, batch_t0, batch_t, batch_y = get_batch(t,a,ic_indices)
            if itr==1:
    ...
    Einterp = Efunc(tspan)
    print(Einterp.shape)
    sol = scipy.integrate.solve_ivp(func_sp,[tspan[0],tspan[-1]],anp[start],t_eval=tspan,max_step=0.01)
    ...
    # ax = plt.subplot(111)
    ax = plt.subplot(241)
    ax.plot(tspan,anp[start:end,0],'o',markersize=1,label='Data')
    ax.plot(tspan,pred_y[:,0],'s',markersize=1,label='Prediction')
    ...
    ax = plt.subplot(242)
    ax.plot(tspan,anp[start:end,1],'o',markersize=1,label='Data')
    ax.plot(tspan,pred_y[:,1],'s',markersize=1,label='Prediction')
    ax.set_ylabel(r'$a_{2}$')

    ax = plt.subplot(243)
    ax.plot(tspan,anp[start:end,2],'o',markersize=1,label='Data')
    ax.plot(tspan,pred_y[:,2],'s',markersize=1,label='Prediction')
    ax.set_ylabel(r'$a_{3}$')

    ax = plt.subplot(244)
    ax.plot(tspan,anp[start:end,3],'o',markersize=1,label='Data')
    ax.plot(tspan,pred_y[:,3],'s',markersize=1,label='Prediction')
    ax.set_ylabel(r'$a_{4}$')

    ax = plt.subplot(245)
    ax.plot(tspan,anp[start:end,4],'o',markersize=1,label='Data')
    ax.plot(tspan,pred_y[:,4],'s',markersize=1,label='Prediction')
    ax.set_ylabel(r'$a_{5}$')
    ax.set_xlabel(r'$t$')

    ax = plt.subplot(246)
    ax.plot(tspan,anp[start:end,5],'o',markersize=1,label='Data')
    ax.plot(tspan,pred_y[:,5],'s',markersize=1,label='Prediction')
    ax.set_ylabel(r'$a_{6}$')
    ax.set_xlabel(r'$t$')

    ax = plt.subplot(247)
    ax.plot(tspan,anp[start:end,6],'o',markersize=1,label='Data')
    ax.plot(tspan,pred_y[:,6],'s',markersize=1,label='Prediction')
    ax.set_ylabel(r'$a_{7}$')
    ax.set_xlabel(r'$t$')

    ax = plt.subplot(248)
    ax.plot(tspan,anp[start:end,7],'o',markersize=1,label='Data')
    ax.plot(tspan,pred_y[:,7],'s',markersize=1,label='Prediction')
    ax.set_ylabel(r'$a_{8}$')
    ax.set_xlabel(r'$t$')
