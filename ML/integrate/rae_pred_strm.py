...
if __name__ == '__main__':
    ...
    PCA_path = '../../BD_FFoRM/c_rods/training_output/na%d_nq_%d_qlim_%.2f_L%.1f_R%.1f/rcspca_%d.p'%(nbins,nqi,qlim,L,R,data_size)
    ...
    I,ornt,cum_weight = pickle.load(open(I_path+'rcsFRR_%.2f_Q2_%.2f_strm_%d.p'%(FRR,Q2,strm),'rb'))
    ornt = ornt*np.pi/180.0
    # ornt = ornt - np.pi
    ...
    ae_path = '../autoencoder/dPCA%d/rmodel%d/'%(dPCA,ae_model)
    ...
    print('ornt shape',ornt.shape)
    ho = np.append(h,ornt[:,np.newaxis],axis=1)
    print('State shape',ho.shape)

    wdir = 'rdh%d/dPCA%d/arch%d_tau%d_alpha%.1e/model%d'%(dh,dPCA,arch,batch_time,alpha,modl)
    ...
    pred_ornt = pred_y[:,-1]
    pred_y = pred_y[:,:-1]
    print('predicted shapes',pred_y.shape,pred_ornt.shape)
    ...
    for window in range(n_win):   
        Irot[window] = ndimage.rotate(Ifill[window],-ornt[window]*180.0/np.pi,reshape=False)

    Ihtr = np.zeros(Ihattilf.shape)
    for window in range(n_win):   
        Ihtr[window] = ndimage.rotate(Ihattilf[window],-pred_ornt[window]*180.0/np.pi,reshape=False)
    ...
    ax = plt.subplot(236)
    plt.plot(tspan,ornt,'o-')
    plt.plot(tspan,pred_ornt,'s--')
    plt.ylabel(r'$\alpha$')
    plt.xlabel(r'$t$')
    ...
    if args.movie == 1:
        ...
        ax3 = plt.subplot(231)
        cax3 = ax3.pcolormesh(qxlist,qylist,Ifill[0],norm=colors.LogNorm(vmin=vmin),cmap=mymap,shading='gouraud')
        ...
        ax4 = plt.subplot(232)
        cax4 = ax4.pcolormesh(qxlist,qylist,Ihattilf[0],norm=colors.LogNorm(vmin=vmin),cmap=mymap,shading='gouraud')
        ...
        cb4.ax.set_title(r'$\hat{\tilde{I}}_R(q)$')

        ax2 = plt.subplot(233)
        # cax2 = ax2.pcolormesh(qxlist,qylist,Itil[0],norm=colors.LogNorm(),cmap=mymap,shading='gouraud')
        cax2 = ax2.pcolormesh(qxlist,qylist,np.sqrt((Ifill[0] - Ihattilf[0])**2),norm=colors.LogNorm(vmin=errmin,vmax=errmax),cmap=mymap,shading='gouraud')
        ax2.set_aspect('equal')
        ax2.set_xlabel(r'$q_x$')
        # ax2.set_ylabel(r'$q_y$')
        cb2 = fig.colorbar(cax2,ax=ax2)
        cb2.ax.set_title(r'$|I_R(q) - \hat{\tilde{I}}_R(q)|$')

        ax5 = plt.subplot(234)
        cax5 = ax5.pcolormesh(qxlist,qylist,Irot[0],norm=colors.LogNorm(vmin=vmin),cmap=mymap,shading='gouraud')
        ...
        ax6 = plt.subplot(235)
        cax6 = ax6.pcolormesh(qxlist,qylist,Ihtr[0],norm=colors.LogNorm(vmin=vmin),cmap=mymap,shading='gouraud')
        # cax4 = ax4.pcolormesh(qxlist,qylist,Ihattil[0],norm=colors.SymLogNorm(linthresh=1e-5, linscale=0.03, base=10),cmap=mymap,shading='gouraud')
        ax6.set_aspect('equal')
        ax6.set_xlabel(r'$q_x$')
        # ax4.set_ylabel(r'$q_y$')
        cb6 = fig.colorbar(cax6,ax=ax6)
        cb6.ax.set_title(r'$\hat{\tilde{I}}(q)$')

        ax7 = plt.subplot(236)
        # cax2 = ax2.pcolormesh(qxlist,qylist,Itil[0],norm=colors.LogNorm(),cmap=mymap,shading='gouraud')
        cax7 = ax7.pcolormesh(qxlist,qylist,np.sqrt((Irot[0] - Ihtr[0])**2),norm=colors.LogNorm(vmin=errmin,vmax=errmax),cmap=mymap,shading='gouraud')
        ax7.set_aspect('equal')
        ax7.set_xlabel(r'$q_x$')
        # ax2.set_ylabel(r'$q_y$')
        cb7 = fig.colorbar(cax7,ax=ax7)
        cb7.ax.set_title(r'$|I(q) - \hat{\tilde{I}}(q)|$')

        plt.tight_layout()

        def animate(i):
           # ax.set_title(r'$\tilde{t} = %.2f$'%(i*avg_win))
           cax3.set_array(Ifill[i])
           cax4.set_array(Ihattilf[i])
           cax2.set_array(np.sqrt((Ifill[i] - Ihattilf[i])**2))
