...
class ODE2(nn.Module):
    ...
    def forward(self, t, y):
        ...
        E1 = aTGafunc(t.detach().numpy() + t0.detach().numpy())
        E2 = gfunc(t.detach().numpy() + t0.detach().numpy())
...
if __name__ == '__main__':
    ...
    print(h.shape)
    Q,phase_t = pickle.load(open(data_dir+'l2c_smooth_na200_nq_100_qlim_0.10_L1680.0_R34.0/labQphase_snaps_%d.p'%data_size,'rb'))
    grad,E = pickle.load(open(data_dir+'l2c_smooth_na200_nq_100_qlim_0.10_L1680.0_R34.0/GdotE_snaps_%d.p'%data_size,'rb'))
    ...
    aTGafunc = interp1d(tnp, aTGa, axis = 0, kind = 'linear', bounds_error=False, fill_value="extrapolate")
    t = torch.tensor(tnp)
