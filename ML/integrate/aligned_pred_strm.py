...
parser.add_argument('--dir', type=str, default='l2c_smooth') 
...
if __name__ == '__main__':
    ...
    I_path = '../../BD_FFoRM/c_rods/training_output/%s_na%d_nq_%d_qlim_%.2f_L%.1f_R%.1f/'%('l2c_smooth',nbins,nqi,qlim,L,R)
    IQa,Q,phase_t,Gd,E = pickle.load(open(I_path+'alignedI_labphase_FRR_%.2f_Q2_%.2f_strm_%d.p'%(FRR,Q2,strm),'rb'))
    ...
    for i in range(E.shape[0]):
        ...
        aTGa[i] = np.transpose(RQ)@Gd[i]@RQ
    ...
    aTGafunc = interp1d(tspan, aTGa, axis = 0, kind = 'linear', bounds_error=False, fill_value="extrapolate")
