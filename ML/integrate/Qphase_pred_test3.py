...
if __name__ == '__main__':
    ...
    # IQa,Q,phase_t,Gd,E = pickle.load(open(I_path+'IQphase.p','rb'))
    IQa,Q,phase_t,Gd,E = pickle.load(open(I_path+'l2c_IQphase.p','rb'))
    print(IQa.shape)
    tspan = np.arange(len(IQa))*dt

    PCA_path = '../../BD_FFoRM/c_rods/training_output/smooth_na%d_nq_%d_qlim_%.2f_L%.1f_R%.1f/Qphase_pca_%d.p'%(nbins,nqi,qlim,L,R,data_size)
