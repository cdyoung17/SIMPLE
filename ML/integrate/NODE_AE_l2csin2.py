...
class ODE2(nn.Module):
    ...
    def forward(self, t, y):
        ...
        phase_sub_Q = -ornt.detach().numpy()/2.0 + thetaQ[:,np.newaxis]
        # print(phase_sub_Q.shape,ornt.shape,thetaQ.shape)
        Rphase = np.zeros((E.shape[0],3,3))
        for i in range(E.shape[0]):
            Rphase[i] = np.array([[np.cos(phase_sub_Q[i]),-np.sin(phase_sub_Q[i]),0.0],[np.sin(phase_sub_Q[i]),np.cos(phase_sub_Q[i]),0.0],[0.0,0.0,0.0]])
        RphaseT = np.transpose(Rphase,axes=[0,2,1])
        # print(Rphase.shape)
        # exit()
        # # Phase-aligned E for pattern evolution
        # E1 = RphaseT@E@Rphase
        E1 = np.zeros((E.shape[0],3,3))
        for i in range(E1.shape[0]):
            E1[i] = RphaseT[i]@E[i]@Rphase[i]
        E1 = np.reshape(E1,(E1.shape[0],9))
        # print(E1.shape,Rphase.shape,RphaseT.shape,E.shape)
        # exit()
