...
if __name__ == '__main__':
    ...
    ntraj = 100000
    stress_path = '../../BD_test_data/output/FT%.2f_Pe%.1e/'%(flowType,Pe)
    tspan,stress,Gd,E = pickle.load(open(stress_path+'t_stress_grad_E_%d.p'%(ntraj),'rb'))
    print(tspan)
    ...
    # gfunc = interp1d(tspan, Gd[:,red_sym_ind], axis = 0, kind = 'linear', bounds_error=False, fill_value="extrapolate")
    ...
    I,cum_weight = pickle.load(open(stress_path+'Iq_%d_%d.p'%(ntraj,nbins),'rb'))
    ...
    plt.tight_layout()
    plt.savefig(open('enc_FT%.2f_Pe%.1e.png'%(flowType,Pe),'wb'))
    # plt.show()
    ...
    if args.movie == 1:
        ...
        ax4.set_title('Lab frame model prediction')
        ...
        cax2 = ax2.pcolormesh(qxlist,qylist,np.sqrt((I[0] - Ihattil[0])**2)/Iq_norm,norm=colors.LogNorm(vmin=errmin,vmax=errmax),cmap=mymap,shading='gouraud')
        ax2.set_aspect('equal')
        ax2.set_xlabel(r'$q_x$')
        # ax2.set_ylabel(r'$q_y$')
        cb2 = fig.colorbar(cax2,ax=ax2)
        cb2.ax.set_title(r'$|I(q) - \hat{\tilde{I}}(q)|/|I(q)|_t$')

        # # ax4 = plt.subplot(132)
        # ax5 = plt.subplot(144)
        # cax5 = ax5.pcolormesh(qxlist,qylist,Itil[0],norm=colors.LogNorm(vmin=vmin),cmap=mymap,shading='gouraud')
        # # cax4 = ax4.pcolormesh(qxlist,qylist,Ihattil[0],norm=colors.SymLogNorm(linthresh=1e-5, linscale=0.03, base=10),cmap=mymap,shading='gouraud')
        ...
        def animate(i):
           ...
           cax2.set_array(np.sqrt((I[i] - Ihattil[i])**2)/Iq_norm)
           # cax5.set_array(Itil[i])
           ax3.set_title('Data '+r'$\tilde{t} = %.2f$'%(i*avg_win))
        ...
        anim.save('FT%.2f_Pe%.1e.mp4'%(flowType,Pe))
        # plt.show()
    ...
    plt.savefig(open('str_FT%.2f_Pe%.1e.png'%(flowType,Pe),'wb'))
    ...
    plt.savefig(open('flow_FT%.2f_Pe%.1e.png'%(flowType,Pe),'wb'))
