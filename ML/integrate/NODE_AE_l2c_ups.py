...
parser.add_argument('--niters', type=int, default=8000)       #Iterations of training
...
parser.add_argument('--dir', type=str, default='l2c_ups_smooth') # Name of directory - use for code changes
parser.add_argument('--ae_model', type=int, default=1)
...
class ODE2(nn.Module):
    ...
    def forward(self, t, y):
        ...
        sina = torch.sin(ornt)
        cosa = torch.cos(ornt)
        # print('sina, cosa shapes',sina.shape,cosa.shape)
        state = torch.cat((h,sina[:,:,np.newaxis]),axis=-1)
        state = torch.cat((state,cosa[:,:,np.newaxis]),axis=-1)
        t0 = y[1]
        # print('NODE training state shape',state.shape)
        # exit()

        E1 = aTEafunc(t.detach().numpy() + t0.detach().numpy())
        E2 = QTEQfunc(t.detach().numpy() + t0.detach().numpy())
        # E = Efunc(t.detach().numpy() + t0.detach().numpy())
        # Q = Qfunc(t.detach().numpy() + t0.detach().numpy())
        # Q = np.reshape(Q,(Q.shape[0],3,3))
        # QT = np.transpose(Q,axes=[0,2,1])
        # thetaQ = np.arctan2(Q[:,0,1],Q[:,0,0])
        # # The phase is computed in the co-rotating frame, but we want it in the lab frame to transform the lab frame E
        # phase_sub_Q = -ornt/2.0 + thetaQ[:,np.newaxis]
        # print(phase_sub_Q.shape,ornt.shape,thetaQ.shape)
        # Rphase = np.array([[np.cos(phase_sub_Q),-np.sin(phase_sub_Q),0.0],[np.sin(phase_sub_Q),np.cos(phase_sub_Q),0.0],[0.0,0.0,0.0]])
        # RphaseT = np.transpose(Rphase)
        # print(Rphase.shape)
        # exit()
        # # Phase-aligned E for pattern evolution
        # E1 = RphaseT@E@Rphase
        # # Co-rotating frame E for phase evolution
        # E2 = QT@E@Q
        # # g2 = QTEQfunc(t.detach().numpy() + t0.detach().numpy())
        E1 = torch.tensor(E1)
        E1 = E1[:,np.newaxis,:]
        ...
        hE1 = torch.cat((state,E1),axis=-1)
        hE2 = torch.cat((state,E2),axis=-1)
        # exit()

        hRHS = self.net(hE1) - self.alpha*h
        thetaRHS = self.netornt(hE2) - self.alpha*ornt[:,:,np.newaxis]
...
if __name__ == '__main__':
    ...
    h = pickle.load(open('../autoencoder/dPCA%d/%s/rmodel%d/encoded_data.p'%(dPCA,ae_dir,ae_model),'rb'))
    print(h.shape)
    # PCA_dir = 'l2c_smooth'
    PCA_dir = args.dir
    Q,phase_t = pickle.load(open(data_dir+PCA_dir+'_na200_nq_100_qlim_0.10_L1680.0_R34.0/Qphase_snaps_%d.p'%data_size,'rb'))
    grad,E = pickle.load(open(data_dir+PCA_dir+'_na200_nq_100_qlim_0.10_L1680.0_R34.0/GdotE_snaps_%d.p'%data_size,'rb'))
    ...
    for i in range(E.shape[0]):
        ...
        # aTEa[i] = np.transpose(RQ)@grad[i]@RQ
    ...
    grad = np.reshape(grad,(grad.shape[0],9))
    ...
    Qfunc = interp1d(tnp, Q, axis = 0, kind = 'linear', bounds_error=False, fill_value="extrapolate")
    QTEQfunc = interp1d(tnp, QTEQ, axis = 0, kind = 'linear', bounds_error=False, fill_value="extrapolate")
    aTEafunc = interp1d(tnp, aTEa, axis = 0, kind = 'linear', bounds_error=False, fill_value="extrapolate")
    t = torch.tensor(tnp)
