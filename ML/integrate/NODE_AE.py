...
parser.add_argument('--dh', type=int, default=3) # Autoencoder latent dimension
...
if __name__ == '__main__':
    ...
    ae_model = 7
    h = pickle.load(open('../autoencoder/dPCA%d/model%d/encoded_data.p'%(dPCA,ae_model),'rb'))
    ...
    h = torch.tensor(h[:data_size,np.newaxis,:])
    ...
    wdir = 'dh%d/dPCA%d/arch%d_tau%d_alpha%.1e/model%d'%(dh,dPCA,arch,batch_time,alpha,model)
    ...
    if args.load==0 or args.load==2:
        ...
        for itr in range(itr_start, args.niters + 1):
            ...
            optimizer.zero_grad()
            batch_y0, batch_t0, batch_t, batch_y = get_batch(t,h,ic_indices)
            if itr==1:
    ...
    # ax = plt.subplot(111)
    ax = plt.subplot(131)
    ax.plot(tspan,h[start:end,0],'o',markersize=1,label='Data')
    ...
    ax = plt.subplot(132)
    ax.plot(tspan,h[start:end,1],'o',markersize=1,label='Data')
    ax.plot(tspan,pred_y[:,1],'s',markersize=1,label='Prediction')
    ax.set_ylabel(r'$h_{1}$')

    ax = plt.subplot(243)
    ax.plot(tspan,h[start:end,2],'o',markersize=1,label='Data')
    ax.plot(tspan,pred_y[:,2],'s',markersize=1,label='Prediction')
    ax.set_ylabel(r'$h_{2}$')
